...
class E2E_PextE(nn.Module):
    def __init__(self, embedding_dim, embedding_dim_pos, sen_len, doc_len, keep_prob1, keep_prob2, \
        ...
        self.cause_bilstm = nn.LSTM(2*n_hidden+50, n_hidden, batch_first=True, bidirectional=True)
        self.pos_bilstm = nn.LSTM(2*n_hidden+50, n_hidden, batch_first=True, bidirectional=True)
    ...
    def forward(self, x, distance, audio):
        s = self.get_clause_embedding(x)
        x_pos, pred_pos = self.get_emotion_prediction(torch.cat([s, audio], -1))
        x_cause, pred_cause = self.get_cause_prediction(torch.cat([s, audio], -1))
...
def train_and_eval(Model, pos_cause_criterion, pair_criterion, optimizer):
    ...
    tr_y_position, tr_y_cause, tr_y_pair, tr_x, tr_sen_len, tr_doc_len, tr_distance, tr_audio, tr_doc_ids = load_data_pair(
                    './data_combine_eng/'+train_file_name, word_id_mapping, max_doc_len, max_sen_len)
    val_y_position, val_y_cause, val_y_pair, val_x, val_sen_len, val_doc_len, val_distance, val_audio, val_doc_ids = \
    ...
    for epoch in range(1, training_epochs + 1):
        ...
        for train, _ in get_batch_data_pair(
            tr_x, tr_sen_len, tr_doc_len, tr_y_position, tr_y_cause, tr_y_pair, tr_distance, tr_audio, batch_size):
            tr_x_batch, tr_sen_len_batch, tr_doc_len_batch, tr_true_y_pos, tr_true_y_cause, \
            tr_true_y_pair, tr_distance_batch, tr_audio_batch = train
            Model.train()
            tr_pred_y_pos, tr_pred_y_cause, tr_pred_y_pair = Model(embedding_lookup(word_embedding, \
            tr_x_batch), embedding_lookup(pos_embedding, tr_distance_batch), tr_audio_batch.to(device))
        ...
        with torch.no_grad():
            ...
            val_x), embedding_lookup(pos_embedding, val_distance), val_audio.to(device))
        ...
        if result_avg_pair[-1] > max_f1_avg:
            torch.save(pos_embedding, "./save/pos_embedding.pth")
            torch.save(Model.state_dict(), "./save/E2E-PextE.pth")
...
def main():
    ...
    audio = torch.rand([batch_size, max_doc_len, 50]).to(device)
    pred_pos, pred_cause, pred_pair = Model(x, distance, audio)
