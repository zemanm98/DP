...
f = open("config/audio_config.json")
...
def audio_emotion_learn(model_name, dataset, feature_extraction):
    ...
    if dataset == "ECF":
        train_x, val_x, train_y, val_y, test_x, test_y = load_ECF(feature_extraction, model_name)
    elif dataset == "RAVDESS":
    ...
    with torch.no_grad():
        ...
        print("Test metrics:\nacc: " + str(acc) + "\nf1: " + str(f1) + "\n")
    torch.save(model.state_dict(), "mfcc_model/" + model_name + "_" + dataset + "_" + feature_extraction + ".pt")
