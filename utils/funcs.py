...
def get_batch_data_pair(x, sen_len, doc_len, y_position, y_cause, y_pair, distance, audio, batch_size, test=False):
    for index in batch_index(len(y_cause), batch_size, test):
        ...
        y_pair[index], distance[index], audio[index]]
...
def create_pred_json(pred_y, true_y, doc_len, doc_ids):
    predict_json = {}
    for i in range(len(doc_len)):
        for j in range (0, doc_len[i]):
            predict_json["" + str(doc_ids[i].item()) + "_" + str(j + 1)] = {"utterance_id": "" + str(doc_ids[i].item()) + "_" + str(j + 1),
                                                                 "emotion_cause_links": []}
        true_y_masked = getmask(true_y[i].clone(), doc_len[i])
        pred_y_masked = getmask(pred_y[i].clone(), doc_len[i])
        _, true_indices_masked = torch.max(true_y_masked, 1)
        _, pred_indices_masked = torch.max(pred_y_masked, 1)
        for index in range(len(pred_indices_masked)):
            value = pred_indices_masked[index].item()
            l = doc_len[i].item()
            x = int((index / l)) + 1
            y = (index % l) + 1
            if value == 1 and y <= x:
                predict_json["" + str(doc_ids[i].item()) + "_" + str(y)]["emotion_cause_links"].append({"source_id":
                                                                                                        "" + str(doc_ids[i].item()) + "_" + str(y),
                                                                                                        "target_id":
                                                                                                        "" + str(doc_ids[i].item()) + "_" + str(x)})

    json_object = json.dumps(predict_json, indent=4)
    with open("pokus3.json", "w") as outfile:
